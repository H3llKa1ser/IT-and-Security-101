...
1) Minimize or avoid single cloud provider lock-in

2) Business units, subsidiaries, or acquired companies have already adopted different cloud platforms

3) Different cloud providers may have regulatory and data sovereignty requirements in different geopolitical regions

4) Improve business continuity and disaster recovery by duplicating workloads across two cloud providers

5) Maximize performance by running applications close to user locations, which may require hybrid or multicloud adoption

6) Enable easy migration for some data platforms or industry-specific applications by adopting multicloud strategies
